...
3. **FPN path** — gated on `DeviceModelName == "GVRD-MRC HighSpeed"`. The first 10 frames are passed through; frame 10 is captured with `ExposureTime=0` and stored as the dark reference (`darknoise_img`); from frame 11 onward, every frame goes through `darknoise_bw_subtract` in `darknoise.cpp` before becoming the "processed" image. Note: the grab loop calls `darknoise_bw_subtract` (the OpenMP variant); `darknoise.cpp` also defines a hand-written SSE2 version, `darknoise_bw_subtract_SSE2`, which is not currently wired in. Both apply the same `(src - dark) * 1.5` transform with optional `SATURATION` skip. The exposure-time juggling around frame 10 is the trick — don't reorder it.
